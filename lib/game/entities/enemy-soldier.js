...
.defines( function() {
  EntityEnemySoldier = EntityEnemyBase.extend({
    ...
    init: function( x, y, settings ) {
      ...
      this.rewardWeapon = new WeaponBeam();
      ...
      if(this.enabled) {
        if( this.hesitationTimer.delta() > 0 ) {
          ...
          if ( this.playerNearby(250) ) {
            ...
            this.flip = !(this.pos.x < playerPos.x);
    ...
    attack: function() {
      if( this.playerNearby(250) && this.weaponFireTimer.delta() > 0 ) {
        var position = {
