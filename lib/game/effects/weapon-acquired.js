...
.defines(function() {
  EffectWeaponAcquired = ig.Entity.extend({
    lifetimeTimer: null,
    lifetime: 3,
    ...
    update: function() {
      ...
      for( var i = 0; i < effectsToSpawn; i++ ) {
        ig.game.spawnEntity(EffectWeaponAcquiredParticle, player.pos.x, player.pos.y);
      }
    },
    ...
    draw: function () {
      var player = ig.game.player;
      var x = ig.game.camera.calculatePosition('x', player.pos.x);
      var y = ig.game.camera.calculatePosition('y', player.pos.y) - ig.game.font.heightForString( 'a' );

      ig.system.context.globalAlpha = this.lifetimeTimer.delta().map(1, this.lifetime, 1, 0);;
      ig.game.font14.draw( this.weaponName + ' acquired!', x, y, ig.Font.ALIGN.CENTER );
      ig.system.context.globalAlpha = 1;
    }
  });

  EffectWeaponAcquiredParticle = ig.Entity.extend({
    size: {x: 4, y: 4},
    maxVel: {x: 300, y: 250},
    lifetime: 1,
    fadetime: 1,
