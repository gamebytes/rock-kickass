...
.defines(function () {
  ig.PauseScreen = ig.Class.extend({
    ...
    draw: function() {
      ...
      font.draw('Weapons:', weaponTextX, weaponTextY, 'left', '#ffffff');
      ...
      for (var i = 0; i < playerController.allWeapons.length; i++) {
        ...
        font.draw(playerController.allWeapons[i].displayName, weaponTextX + (100 / ig.system.scale) + 20, weaponTextY + ((i + 1) * weaponTextSpacing), 'left', '#ffffff');
      ...
      for (var i = 0; i < this.menu.length; i++) {
        font.draw(this.menu[i].display(), menuX, menuY + i * menuSpacing, 'left', '#ffffff');
      };
    }
  });
});