...
MyGame = ig.Game.extend({
  ...
  clearColor: "#FFFFFF",
    ...
    if(LIGHTING_ENABLED) this.lightManager = new ig.LightManager('rgba(0,0,0,0.8)', [0,0,0, 255 * 0.8]);
  ...
  bindInput: function() {
    ...
    if(LIGHTING_ENABLED) ig.input.bind( ig.KEY.F, 'light' ); // flashlight toggle
  },
  ...
  loadLevel: function( level ) {
    ...
    if(LIGHTING_ENABLED)  this.lightManager.update();
	},

	draw: function() {
    if(LIGHTING_ENABLED) this.lightManager.drawLightMap();
    ...
    if(LIGHTING_ENABLED) this.lightManager.drawShadowMap();
	}
});
